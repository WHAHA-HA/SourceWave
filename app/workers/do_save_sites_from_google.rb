class SaveSitesFromGoogle
  ...
  def perform(crawl_id, options = {})
    if Rails.cache.read(['do_running_crawls']).to_a.include?(crawl_id)
      ...
      urls_array.each do |u|
        ...
        site_id = site.id
        $redis.sadd "do_all_ids/#{crawl_id}", ["do_site/#{site_id}/processing_batches/total", "do_site/#{site_id}/broken_domains", "do_site/#{site_id}/processing_batches/finished", "do_site/#{site_id}/expired_domains", "do_site/#{site_id}/processing_batches/running", "do_site/#{site_id}/total_site_urls"]
      end
    else
