class DoGatherLinks
  ...
  def perform(site_id, maxpages, base_url, max_pages_allowed, crawl_id, options={})
    ...
    Retriever::PageIterator.new("#{base_url}", opts) do |page|
      total_crawl_urls = Rails.cache.read(["do_crawl/#{crawl_id}/urls_found"], raw: true).to_i
      
      links = page.links
      links_count = links.count.to_i
      
      Rails.cache.increment(["do_crawl/#{crawl_id}/urls_found"], links_count)
      Rails.cache.increment(["do_site/#{site_id}/total_site_urls"], links_count)
      ...
      if process == true
        redis_id = ("do_process-#{crawl_id}-"+SecureRandom.hex+Time.now.to_i.to_s)
        ...
        $redis.sadd 'do_all_processing_ids', redis_id
        ...
        if $redis.smembers('do_all_processing_ids').count <= 1
  ...
  def on_complete(status, options)
    ...
    if !batch.nil?
      total_crawl_urls = Rails.cache.read(["do_crawl/#{crawl.id}/urls_found"], raw: true).to_i
  ...
  def self.start(options = {})
    ...
    if running_crawl.gather_links_batches.where(status: 'pending').count > 0
      ...
      gather_links_batch.on(:complete, DoGatherLinks, 'bid' => gather_links_batch.bid, 'crawl_id' => options["crawl_id"], 'site_id' => site.id, 'processor_name' => processor_name)
      Crawl.using("#{processor_name}").update(running_crawl.id, status: 'running')
      gather_links_batch.jobs do
        puts 'starting to gather links'
        DoGatherLinks.perform_async(site.id, site.maxpages, site.base_url, running_crawl.max_pages_allowed, options["crawl_id"], 'processor_name' => processor_name)
      end
