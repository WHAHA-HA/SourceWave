...
class VerifyNamecheap
    ...
    if !page_from_redis.nil?
      ...
      begin
        ...
        if !url.domain.empty? && !url.public_suffix.empty?
          ...
          if !Rails.cache.read(["do_crawl/#{page['crawl_id']}/checked"]).include?(parsed_url)
            puts "checking url #{parsed_url} on namecheap"
            avl = Set.new(Rails.cache.read(["do_crawl/#{page['crawl_id']}/checked"]))
            Rails.cache.write(["do_crawl/#{page['crawl_id']}/checked"], avl.add("#{parsed_url}").to_a)
            ...
            tlds = [".gov", ".edu", ".nl"]
            if domain_is_available == true && !Rails.cache.read(["do_crawl/#{page['crawl_id']}/available"]).include?("#{parsed_url}") && !tlds.any?{|tld| parsed_url.include?(tld)}
            # if json['available'].to_s == 'true' && !Rails.cache.read(["crawl/#{page['crawl_id']}/available"]).include?("#{parsed_url}") && !tlds.any?{|tld| parsed_url.include?(tld)}
              ...
              set = Set.new(Rails.cache.read(["do_crawl/#{page['crawl_id']}/available"]))
              Rails.cache.write(["do_crawl/#{page['crawl_id']}/available"], set.add("#{parsed_url}").to_a)
              ...
              Rails.cache.increment(["do_crawl/#{page['crawl_id']}/expired_domains"])
              Rails.cache.increment(["do_site/#{page['site_id']}/expired_domains"])
  ...
  def on_complete(status, options)
    ...
    running = Rails.cache.read(['do_running_crawls']).to_a
  ...
  def self.start(options={})
    ...
    expired_rotation = Rails.cache.read(['do_expired_rotation']).to_a
    ...
    all_expired_ids = $redis.smembers('do_all_expired_ids').select{|objs| objs.include?("expired-#{next_crawl_to_process}")}.to_a
    ...
    if !all_expired_ids.empty?
      ...
      Rails.cache.write(['do_domain_being_verified'], [next_expired_id_to_verify])
      puts "the domain to be verified is #{next_expired_id_to_verify}"
      new_expired_rotation = expired_rotation.rotate
      Rails.cache.write(['do_expired_rotation'], new_expired_rotation)
      $redis.srem 'do_all_expired_ids', next_expired_id_to_verify
